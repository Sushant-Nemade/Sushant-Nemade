# Option 2 Changelog

## 2026-08-31

- Preserved the published profile as `README.option-1.md`.
- Defined the Applied AI Dossier visual direction.
- Added isolated Option 2 configuration, generators, assets, preview, validation, and workflows.
- Added dark/light rendering for all theme-sensitive assets.
- Used verified profile configuration, contribution data, and public repository metadata only.
- Added activation and rollback documentation.
